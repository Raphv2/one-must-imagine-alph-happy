...
void Mesh::move_vertex( Point pos )
{
    
    int sizePos = m_positions.size();
    int sizeTex = m_texcoords.size();
    int sizeNormal = m_normals.size();
    Point position = pos;

    m_update_buffers= true;
    
    for(int i = 0; i < sizePos; i++){
        m_positions[i].x += position.x ;
        m_positions[i].y += position.y ;
        m_positions[i].z += position.z ;
    }
    for(int i = 0; i < sizeTex; i++){
        m_texcoords[i].x += position.x;
        m_texcoords[i].y += position.y;
    }
...
TriangleData Mesh::triangleScene( const unsigned int id , Orbiter m_camera) const
{
    ...
    Transform model = projection * view * model;
    unsigned int a, b, c;
    if(m_indices.size() > 0)
    {
        assert((size_t) id*3+2 < m_indices.size());
    ...
    TriangleData triangle;
    triangle.a= model(Point (m_positions[a].x, m_positions[a].y, m_positions[a].z));
    triangle.b= model(Point (m_positions[b].x, m_positions[b].y, m_positions[b].z));
    triangle.c= model(Point (m_positions[c].x, m_positions[c].y, m_positions[c].z));
    
    if(m_normals.size() == m_positions.size())
    {
        triangle.na= model(Point( m_normals[a].x, m_normals[a].y, m_normals[a].z));
        triangle.nb= model(Point( m_normals[b].x, m_normals[b].y, m_normals[b].z));
        triangle.nc= model(Point( m_normals[c].x, m_normals[c].y, m_normals[c].z));
    }
    else
    {
        // calculer la normale geometrique
